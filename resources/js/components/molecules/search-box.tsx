...
export default function SearchBox(props: SearchBoxProps) {
    ...
    return (
        <div hidden={props.hidden} className="w-full xl:w-1/3 bg-white rounded-md flex flex-col xl:m-5 space-y-5 p-6 xl:h-full h-screen shadow-md shadow-warm-gray">
            ...
            <Button className="cursor-pointer rounded-full w-11/12 xl:w-full text-white uppercase font-bold p-2 self-center bottom-[50px] fixed xl:relative xl:bottom-0" disabled={!state.query} onClick={() => props.onSearch(state)}>Search</Button>
        </div>
    )
}
